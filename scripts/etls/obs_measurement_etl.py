...
class ObserMeasurement(ETLEntity):
    ...
    def _set_source_values(self):
        ...
        self._source_data['measurement_source_value'] = self._source_data['description']
        ...
        self._source_data['measurement_type_concept_id'] = self._source_data['category'].map(lambda x: self.CATEGORY_MAP.get(x, x))
        self._source_data['value_as_number'] = self._source_data['value'].astype(float)
        self._source_data['value_source_value'] = self._source_data['value']
